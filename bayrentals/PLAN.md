...
- **Wave 3 — Adversarial verification (parallel skeptic agents, loop until dry).**
  ...
  - Booking race-condition review (exclusion constraint actually the last guard?
    idempotency actually idempotent under concurrent retries?)
  - Stripe object-lifecycle audit: hold expiry/re-auth, webhook retry idempotency,
    refund states, off-session charge failure paths
...
- `client_request_id uuid unique nullable` — idempotency key generated by the client;
  a double-tap or a retry after a network timeout finds the existing booking instead
  of creating a second one
- `notes text`, `created_by uuid`, `created_at timestamptz`, `updated_at timestamptz`
  (bumped by trigger; clients send it back on edits for optimistic concurrency —
  a stale write gets a friendly "this booking changed, reloading" instead of silently
...
- **Critical constraint — no double booking, enforced by the database itself:**
  ...
  ```
- **Live calendars, done correctly:** Supabase Realtime `postgres_changes` respects
  RLS, so customers would never receive *other people's* booking events — subscribing
...
- `business_timezone text default 'America/Los_Angeles'` — ALL day boundaries,
  weekend rules, and season windows are computed in this timezone, never UTC and
  never the phone's locale
- `min_renter_age int`, `grace_period_minutes int`
- `turnaround_minutes int` — prep/cleaning buffer required between consecutive
  bookings of the same car; availability and quotes treat the buffer as booked
- `min_lead_minutes int`, `max_advance_days int` — how soon and how far out a
  booking may start
...
1. **`quote`** — the pricing engine's public face. Input: car_id, date range, extra ids.
   ...
   available *including the turnaround buffer*? within min-lead/max-advance windows?
   renter age ok? license on file?). Used by the app's booking flow, the MyTrips
   timeline, and the website widget — everyone shows the same numbers. Pure function
   over `rate_rules` + `policies` + `extras`; heavily unit-tested.
2. **`create-booking`** — input: car_id, date range, extras, customer info, and a
   client-generated `client_request_id` (idempotency: a retry or double-tap returns
   the existing booking, never creates a second). Re-runs the quote server-side
...
10. **`scheduled-runner`** — invoked by pg_cron **every 5 minutes** (cheap; each task
    ...
    - flags **overdue returns** (past `end_at` + grace period, still `active`) to staff
      and the customer;
    - **re-authorizes deposit holds on long rentals**: Stripe cancels uncaptured
      manual-capture PaymentIntents after ~7 days, so any active rental whose hold is
      nearing expiry gets a fresh hold placed against the saved card before the old
      one is released — otherwise every week-plus rental would silently lose its
      deposit protection mid-trip;
...
- **`ReturnWizardView`** — mirror image: odometer + fuel (validated: return odometer
  must be ≥ checkout, fuel 0–8/8, and any delta over 1,000 miles asks "really?" before
  accepting — one fat-fingered digit must not produce a $400 mileage charge) → guided
  walkaround →
...
- **`ClaimsView`** — open claims list: attach estimate, charge (deposit capture or
  saved card via Stripe; if an off-session charge fails or requires authentication,
  fall back to emailing the customer a Stripe payment link — the claim tracks it
  either way), waive, close. Every claim links its evidence photos.
...
- **Offline queue:** a disk-persisted operation queue (photos, status changes) with
  ...
  Queued files (license shots, walkaround photos) are written with iOS Data
  Protection (`.completeUntilFirstUserAuthentication`) — a stolen phone yields
  ciphertext, not customer documents.
- **Availability updates:** customer screens and the widget subscribe to the
  sanitized availability broadcast (§7); staff screens use `postgres_changes` under
  their role. Both fall back to refetch-on-foreground so a missed event can never
  strand a stale calendar.
...
- **Timezone-explicit:** every day boundary, weekend test, and season window is
  computed in `policies.business_timezone` — never UTC, never the device locale. A
  Friday-night pickup is a weekend night because it's Friday in the Bay Area, no
  matter where the customer's phone thinks it is.
- **Test-first:** this module gets exhaustive Deno unit tests (boundary days, DST
  transitions — including a rental spanning the spring-forward night, overlapping
  season+weekend, discount thresholds, turnaround-buffer edges) before any UI uses it.
...
**god mode bends policy, never physics.** Two more hard rules: every server action
**re-verifies the caller's session and admin role server-side** before touching the
service-role key (a leaked action URL is useless without an admin session), and every
edit carries the row's `updated_at` for optimistic concurrency — two people editing
the same booking get a friendly "changed underneath you, reloading" instead of a
silent overwrite.
...
| Double-tap / retried "Book" request | `client_request_id` idempotency: the retry returns the same booking and PaymentIntent, never a duplicate |
| Deposit hold expires mid-rental (Stripe cancels uncaptured holds ~7 days) | Scheduled runner re-authorizes against the saved card before expiry; reconciliation catches any miss |
| Signed contract email lands in spam | SPF/DKIM verified on bayrentals.com before launch; every document also lives in the app (MyTrips) with a resend button; delivery failures surface in `notifications_log` |
...
error; double-tapping Book creates exactly one booking; an attempt to book inside
another booking's turnaround buffer is refused with the next truly-available time
shown; a first-time guest books with only email + OTP; stale pending bookings expire
and free the dates.
